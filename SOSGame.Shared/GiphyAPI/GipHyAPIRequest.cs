...
namespace SOSGame.Shared.GiphyAPI;

public static class GipHyAPIRequest
{
    private const string APIKEY = "KSkiMTWvZuQ3ZVcNcJlHaKhT375N1TZ6";
    private static readonly string _baseUrl = $"https://api.giphy.com/v1/gifs/search?api_key={APIKEY}&";
    private static HttpClient httpClient = new ();
    public static dynamic SearchWinnerImage()
    {
        try
        {
            HttpResponseMessage response = httpClient
            .GetAsync($"{_baseUrl}q=winner&limit={new Random().Next(1, 40)}" +
            $"&offset={new Random().Next(1, 10)}&rating=g&lang=en").GetAwaiter().GetResult();

            response.EnsureSuccessStatusCode();

            string serializedResult = response.Content.ReadAsStringAsync().Result;

            Root myDeserializedClass = JsonConvert.DeserializeObject<Root>(serializedResult);
